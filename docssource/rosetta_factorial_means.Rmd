...
The results are similar but not very close. Despites the fact that both software indicates the same effect size and the same degrees of freedom, PAMLj give a slightly larger required N because PAMLj takes into the account that the whole model  has three factors, whereas this information cannot be passed to GPower. In fact, PAMLj yields the same results of GPower if one select the option `Liberal` in the `Non-centrality parameter`. The option `Liberal`, in fact, estimate the required N without considering the whole model degrees of freedom, but only the effect degrees of freedom. 
