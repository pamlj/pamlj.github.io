...
`r pic("rosetta/pics/mixed/r1.data.1.png")`
...
`r pic("rosetta/pics/mixed/output.rm.1.png")`
...
`r pic("rosetta/pics/mixed/input.rm.2.png")`

obtaining a sample size of $N=39$.

`r pic("rosetta/pics/mixed/output.rm.2.png")`

With the original $N=50$ cases, the expected power would be $1-\beta=.965$.

`r pic("rosetta/pics/mixed/output.rm.3.png")`
...
`r pic("rosetta/pics/mixed/output.lme.1.png")`

So, the RM-ANOVA results are equivalent to a mixed model with three contrast variables representing _time_, with coefficients $.5052$,$-.2952$, and $-.0043$. The random intercept variance was $\sigma_I^2=.00863$ and the residual variance $\sigma^2=1.009$. We can plug these values in the mixed model power analysis command.

`r pic("rosetta/pics/mixed/input.lme.2.png")`

First, we use the `squared-brackets` syntax to attach three coefficients to the corresponding contrast variables representing _time_. Second, we multiply the random intercept (`1`) by the expected variance ($.0083$). This way, we pass the expected coefficients required to build the mixed model for simulating power and finding the required N. 
...
`r pic("rosetta/pics/mixed/input.lme.3.png")`

We then specify in the `r opt_label("Model Structure")` -> `r opt_label("Clustering vriables")` that the variable _id_ (the participants) has `N per cluster` equal to 4, because within each participant there will be 4 repeated measures.

Then, we specify that the variable _time_ is a categorical variable with 4 levels.

`r pic("rosetta/pics/mixed/input.lme.4.png")`

Finally, we set the residual variance of the model to $1.009$ as we found in the mixed model.

`r pic("rosetta/pics/mixed/input.lme.5.png")`
...
`r pic("rosetta/pics/mixed/output.lme.2.png")`
...
`r pic("rosetta/pics/mixed/output.lme.3.png")`
