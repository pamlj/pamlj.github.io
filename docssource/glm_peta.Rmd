...
Let's consider a scenario where we aim to estimate the required sample size (N) for a planned research project. Assuming we have a single continuous  independent variable with an expected Partial Eta-squared of 0.15 evaluated at α = 0.05. We aim for a power of 0.90. Because the independent variable is continuous, its test will have 1 _df_. Because there is only one independent variable, the `Model degrees of freedom` will be 1 as well (see below for more about computing _df_ in complex models).
