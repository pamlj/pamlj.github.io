...
Results, shown here in the vicinity of $power=.80$ (4th column) shows that the required N (2nd column) is around 96-97, congruent with the results of `r modulename()`
...
The `pwrss` package provides all three method to estimate expected power.
...
```
Results are very close the the ones obtained with `r modulename()`. As a further check for the `Monte Carlo` method, we can employ [Monte Carlo Power Analysis](https://schoemanna.shinyapps.io/mc_power_med/).
