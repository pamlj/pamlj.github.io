...
suggests to use a different adjustment. The adjustment yields values that are usually in between the observed effect size and the population effect size. `r modulename()` provides the possibility to use the GPower adjustment, by simply selecting `r opt("G*Power")` in the `r opt("Use as effect size ")` list. 
